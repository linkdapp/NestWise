...
**SOP: Data Guard Standby (`usatclust2`), a 2-node physical standby for `apexdb`, on Oracle Linux 7**
...
(this page)** covers post-standby validation.
...
Proves Data Guard's switchover protection under real application load rather than
an idle SQL or DGMGRL check. The evidence is a Swingbench throughput chart that
dips and recovers across a live switchover, using this project's standing toolkit
(Swingbench 2.8.0.1630, driven from a Windows client). This is the difference
between "failover completed" as a log line and "the application barely noticed" as
something you can show someone.
...
2. Configure and run Swingbench's `SOE_Client_Side_AC` benchmark, the Order Entry
   workload built specifically to exercise Application Continuity's replay and
   reconnect behaviour, rather than a generic workload with no recovery logic.
3. Connect via a client-side connect string that lists **both** clusters' SCAN
   ...
   single-cluster connect string does not survive a real switchover here.
4. Establish a stable baseline (16 concurrent users) and confirm steady TPS, DML
   ops and response time before touching anything.
5. Trigger a real switchover mid-load via the confirmed `dataguard_switchover_test`
   role (Part 2, Section 14).
6. Watch and capture the throughput chart through the switchover.
7. Verify the switchover at the database level via that same role's own real SQL
   and `srvctl status service` checks, not just DGMGRL's own report.
8. Confirm Swingbench's own recovery, with TPS, DML ops and response time
   returning to baseline without any manual client-side intervention.

**Debugging journey. This changed the actual test setup.** The first real attempt
used a connect string pointing at a single cluster's SCAN only
(`//scan-usatclust1.usat.com:1521/apexdb_rw`). That worked fine for the baseline
load, but the moment a real switchover moved `apexdb_rw` onto `usatclust2` (this
lab's standby cluster, with its own separate SCAN listener), every reconnect
attempt kept hitting `usatclust1`'s listener, which no longer had the service
registered. The result was an endless `SQLRecoverableException` loop with no
...
Root-caused and fixed with a two-address connect descriptor listing both
clusters' SCAN listeners under one `DESCRIPTION`, in
[`tnsnames-application.ora`](tnsnames-application.ora). It is the same pattern any real
...
`SOE_Client_Side_AC`, the config built to demonstrate replay and reconnect
behaviour through an outage rather than a plain connection pool with no recovery
logic of its own.

**Real run, database and Ansible level.** `dataguard_switchover_test` (Part 2's
...
Switchover output. DGMGRL reports success, but the immediate `SHOW
...
This is expected and momentary, not a failure. The role does not trust this message
alone:
...
Role-based service verification across both clusters, via `srvctl status service`.
This is the part that matters for this section, confirming Section 13's services
relocate automatically on a real switchover rather than only in theory:
...
Final role summary and recap, `failed=0` on this run:
...
[`scripts/monitor_dataguard.sh`](scripts/monitor_dataguard.sh) script. It is
role-aware, detecting PRIMARY against PHYSICAL STANDBY locally and skipping
standby-only checks such as MRP status when run against a primary, so the same
script runs unmodified on either side of a switchover. Run against the new standby
here, with both instances open, applying, and no gaps:
...
response time all show a dip around 14:57 to 14:58, with response time spiking
briefly and throughput dropping to near zero, then recovering on their own with no
manual reconnect, back to baseline levels by roughly 14:59. `Logged on Users` never
dropped from 16 throughout.
...
**What this confirms, stated in RPO and RTO terms.** This was a planned switchover
rather than an unplanned failover. The two numbers that matter for a Data Guard SLA
claim are Recovery Point Objective (how much data is lost) and Recovery Time
Objective (how long the outage lasts).

RPO here is zero. `MaxAvailability` protection mode with `LogXptMode=SYNC` means the
standby cannot fall behind the primary by design, and DGMGRL will not begin a
switchover unless the standby is fully caught up. Part 2, Section 15's
`SHOW FAST_START FAILOVER` output already confirmed this configuration as "Enabled
in Zero Data Loss Mode." No committed transaction was lost. That claim is not new
here; this test only had to avoid contradicting it.

RTO is the harder number, and it is the one this test measured. Not DGMGRL's own
"switchover succeeded" timestamp, but the client-observed recovery time from the
Swingbench chart: roughly a minute of measurable application impact, with
throughput dropping to near zero and response time spiking, between the switchover
being triggered and the application recovering on its own with no manual reconnect.
That is the RTO this evidence supports. It is not zero, but it is bounded and
self-healing. The corrected two-cluster connect string plus an Application
Continuity-aware client did not buy a smaller RTO number by itself. It bought the
difference between an RTO of about a minute and an RTO of never, which is the
permanent stall the single-cluster connect string produced on the first attempt.

**Scope.** This measured RTO applies to a **planned switchover**. An **unplanned
failover** is governed by a different mechanism, `FastStartFailoverThreshold`, set
to Oracle's default of 30 seconds in Part 2, Section 15, and has not been measured
under load yet. That is the still-open `dataguard_fsfo_test` work. "The app barely
noticed" is accurate as "recovered on its own within about a minute, confirmed
under a real planned switchover." It is not a claim that the application did not
